...
**버전**: 1.10.0 | **최종 업데이트**: 2026-02-20

---

## v1.10.0 주요 변경

- **Poietes 스킬 신규**: 에로스 사이클 기반 기획 컨설팅 — 소크라테스 21개 질문을 결핍→욕망→다이몬→출산 구조로 재배열
- **tmux 병렬 모드**: auto-orchestrate, ultra-thin-orchestrate에 독립 프로세스 기반 병렬 실행 (`--tmux`) 추가
- **에이전트/훅 안정화**: 전문가 에이전트 및 훅 시스템 전반 품질 개선
...
| **Eros** | `/eros` | 플라톤 향연 기반 결핍 인식 → 디오티마 사다리 → 창작 **(NEW!)** |
